...
class DatabaseManager:
    ...
    async def connect_to_database(self, uri: str):
        ...
        try:
            ...
            # --- الإضافة الجديدة: مجموعة قنوات النشر ---
    ...
    async def initialize_defaults(self):
        ...
        defaults = {
            ...
            # --- الإضافة الجديدة: نصوص واجهة منشورات القناة ---
            "cp_menu_title": "📰 *إدارة منشورات القناة*",
            "cp_set_auto_msg_button": "✍️ تعيين الرسالة",
            "cp_view_auto_msg_button": "👀 عرض الرسالة",
            "cp_publish_now_button": "🚀 نشر الآن",
            "cp_ask_for_auto_msg": "📝 أرسل الآن الرسالة التي سيتم نشرها في القنوات.",
            "cp_auto_msg_set_success": "✅ تم حفظ الرسالة بنجاح.",
            "cp_no_auto_msg": "لم يتم تعيين رسالة بعد.",
            "cp_auto_msg_deleted_success": "🗑️ تم حذف الرسالة.",
            "cp_publish_started": "🚀 جاري النشر إلى `{count}` قناة...",
            "cp_publish_finished": "🏁 اكتمل النشر!\n\n✅ نجح: `{success}`\n❌ فشل: `{failed}`",
            "cp_error_no_auto_msg_to_publish": "⚠️ لا يمكنك النشر لأنه لم يتم تعيين رسالة تلقائية بعد!",
            "cp_error_no_channels_to_publish": "⚠️ لا يمكنك النشر لأنه لا توجد قنوات مضافة!",
        }
    ...
    # --- (وظائف الردود التلقائية والتذكيرات موجودة هنا دون تغيير) ---
    ...
    # --- الإضافة الجديدة: وظائف منشورات القناة ---
    async def set_auto_publication_message(self, message_data: dict):
        """يحفظ أو يحدث الرسالة التلقائية للنشر."""
        if not self.is_connected(): return
        await self.settings_collection.update_one(
            {"_id": "auto_publication_message"},
            {"$set": {"message": message_data}},
            upsert=True
        )

    async def get_auto_publication_message(self):
        """يجلب الرسالة التلقائية للنشر."""
        if not self.is_connected(): return None

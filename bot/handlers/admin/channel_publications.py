...
async def show_cp_menu(call: types.CallbackQuery, state: FSMContext):
    """Displays the main menu for managing channel publications."""
    await state.finish()
...
async def set_auto_msg_start(call: types.CallbackQuery):
    """Starts the process of setting the auto-publication message."""
...
async def set_auto_msg_received(message: types.Message, state: FSMContext):
    """
    Receives and saves the auto-publication message.
    This is the modern and correct way to save a message for copying.
    """
    # We save the chat_id and message_id of the original message.
    # This allows us to use bot.copy_message for a perfect copy.
    saved_message_data = {
...
async def view_auto_msg(call: types.CallbackQuery, bot: Bot):
    """Displays the currently set auto-publication message."""
    ...
    # Use bot.copy_message for a reliable copy.
    keyboard = types.InlineKeyboardMarkup().add(
...
# --- 4. Publish Now Flow ---
async def publish_now(call: types.CallbackQuery, bot: Bot):
    """Initiates the publishing process to all channels."""
    auto_message_data = await db.get_auto_publication_message()
    if not auto_message_data:
...
def register_channel_publications_handlers(dp: Dispatcher):
    """Registers all handlers for the channel publications feature."""
